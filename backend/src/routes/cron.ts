...
export const initCronJobs = async () => {
  try {
    ...
    // Ensure Auto-Fetch Sync job exists
    const autoFetchJob = jobsConfig.find(j => j.name === 'Auto-Fetch Data Sync');
    if (!autoFetchJob) {
      const newJob: CronJob = {
        id: 'auto_fetch_sync',
        name: 'Auto-Fetch Data Sync',
        schedule: '0 */1 * * *', // Every 1 hour
        command: 'echo "Syncing data from integrations..." > /media/tokio/_dde_home/tokyo-os/data/raw/manual_uploads/last_sync.log',
        enabled: true
      };
