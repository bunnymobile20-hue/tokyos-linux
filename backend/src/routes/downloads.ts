...
router.post('/config', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Falha ao salvar o diretório de download' });
  }
});
...
router.get('/status', async (_req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      data: {
        available: true,
        message: 'O mecanismo de download está funcionando normalmente',
    ...
    res.json({
      ...
      data: {
        available: false,
        message: 'Não foi possível conectar ao mecanismo de download，Por favor confirme Aria2 Correndo。',
        version: '',
        downloadDir,
        globalStat: {
          downloadSpeed: '0',
...
router.get('/tasks', async (_req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        available: false,
        message: 'A tarefa de download não pode ser lida no momento，Verifique a conexão do mecanismo de download。',
        tasks: [],
...
router.post('/create', async (req, res) => {
  try {
    ...
    if (!url) {
      return res.status(400).json({ success: false, error: 'Por favor preencha o link de download' });
    }
    ...
    res.json({
      ...
      data: {
        gid,
        statusHint: isMagnet ? 'Na fila，Obtendo metadados magnéticos。' : 'Já adicionado à fila de download。'
      }
    });
  } catch (error: any) {
    logger.error(`Aria2 create task error: ${error.message}`, { module: 'Downloads' });
    res.status(500).json({ success: false, error: 'Falha ao criar tarefa de download' });
  }
});
...
router.post('/task/:gid/action', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Falha ao executar a operação de tarefa de download' });
  }
});
...
router.post('/cleanup', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Falha ao limpar o histórico de downloads' });
  }
});
...
router.post('/file', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Falha ao excluir o arquivo local' });
  }
});
