...
// Dynamic import for music-metadata due to ESM/CommonJS compatibility
let mm: any = null;
...
const warmTrackMetadata = async (track: LocalTrack) => {
  ...
  if (!match) {
    ...
    track.warmupFailureReason = 'Não foram encontrados resultados correspondentes suficientes do NetEase Cloud';
    return track;
  }
    ...
    for (const file of files) {
      ...
      if (existing && existing.path === file) {
        ...
        try {
          ...
          const metadata = await musicMetadata.parseFile(file, { duration: true, skipCovers: false });
