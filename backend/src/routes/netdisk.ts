...
async function listAListPath(targetPath: string) {
  ...
  if (!token) {
    return {
      ...
      message: 'Não é possível fazer login no plano de fundo de montagem subjacente AList，Por favor confirme primeiro AList Iniciado com senha de administrador ainda padrão。'
    };
  }
  ...
  try {
    ...
    if (data.code === 401 || data.code === 403) {
      ...
      return {
        ...
        message: 'AList O status de login expirou，Insira novamente o histórico de montagem subjacente e tente novamente.。'
      };
    }
    ...
    if (message.includes('object not found') || message.includes('not found') || message.includes('file not found')) {
      return {
        ...
        message: `O ponto de montagem ainda não foi detectado ${targetPath}，ilustrar ${targetPath === '/baidu' ? 'Baidu SkyDisk' : 'Disco de rede Quark'} Ainda não está lá AList Configuração concluída。`
      };
    }
    ...
    return {
      ...
      message: String(data.message || 'AList Um erro desconhecido foi retornado')
    };
  } catch (error: any) {
    ...
    return {
      ...
      message: 'Não é possível conectar-se ao back-end de montagem subjacente AList，Por favor confirme primeiro AList serviço está em execução。'
    };
  }
}
...
async function alistAdminRequest<T>(pathname: string, init?: RequestInit): Promise<T> {
  const token = await getAlistToken();
  if (!token) {
    throw new Error('Não é possível fazer login no plano de fundo de montagem subjacente AList');
  }
  ...
  if (data.code === 401 || data.code === 403) {
    alistToken = '';
    throw new Error('AList O status de login expirou，Por favor, tente novamente mais tarde');
  }

  if (data.code !== 200) {
    throw new Error(String(data.message || 'AList A interface de gerenciamento retornou um erro desconhecido'));
  }
...
router.post('/configure', async (req, res) => {
  ...
  if (!credential) {
    return res.status(400).json({
      success: false,
      error: brand === 'baidu' ? 'Preencha o Baidu Netdisk refresh_token' : 'Preencha o Quark Network Disk cookie'
    });
  }
  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        mounted: status.ok,
        message: status.ok ? 'A montagem do disco de rede foi configurada automaticamente.' : status.message
      }
    });
  } catch (error: any) {
