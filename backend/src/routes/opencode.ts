...
router.post('/unlock', (req, res) => {
  if (!hasOpenCodeAppLockPassword()) {
    ...
    return res.status(500).json({ success: false, error: 'OpenCode O bloqueio de aplicativos não está configurado' });
  }

  const password = typeof req.body?.password === 'string' ? req.body.password : '';
  if (password !== OPENCODE_APP_LOCK_PASSWORD) {
    logger.warn('OpenCode app unlock failed', { module: 'OpenCode' });
    return res.status(401).json({ success: false, error: 'OpenCode Senha de bloqueio de aplicativo errada' });
  }
