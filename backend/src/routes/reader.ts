...
router.post('/feeds', async (req, res) => {
  try {
    ...
    if (!url) {
      return res.status(400).json({ success: false, error: 'RSS URL não pode estar vazio' });
    }

    const category = (req.body?.category || 'Sem categoria') as ReaderCategory;
    const feed = await createReaderFeed(await getReaderDir(), {
      name: String(req.body?.name || '').trim() || 'Feed personalizado',
      url,
      category,
    });
...
router.post('/articles/:id/summarize', async (req, res) => {
  try {
    ...
    if (!article) {
      return res.status(404).json({ success: false, error: 'A informação não existe' });
    }
