...
function normalizeArticleRecord(article: ReaderArticle): ReaderArticle {
  return {
    ...article,
    author: typeof article.author === 'string' ? article.author : 'fonte desconhecida',
...
async function summarizeArticleWithAi(article: ReaderArticle) {
  ...
  const response = await requestOpenClawChatCompletion([
    {
      role: 'system',
      content: 'Você é um assistente de edição de informações。Por favor, refine as informações de entrada em 3 Um resumo chinês conciso，Uma frase por item。Somente devolução JSON，O formato deve ser {"summary":["...","...","..."]}。Não produza markdown，não explique。',
    },
    {
      role: 'user',
      content: `título：${article.title}\nClassificação：${article.category}\ntexto：\n${sourceText}`,
    },
...
export async function createReaderFeed(readerDir: string, input: Pick<ReaderFeed, 'name' | 'url' | 'category'>): Promise<ReaderFeed> {
  ...
  const feed: ReaderFeed = {
    id: `custom-${Date.now()}`,
    name: input.name.trim() || 'Feed sem nome',
    url: input.url.trim(),
...
export async function translateReaderArticle(readerDir: string, articleId: string) {
  ...
  const sourceText = article.contentText || article.summary.join('\n');
  if (!sourceText.trim()) {
    throw new Error('A postagem atual não tem conteúdo de texto traduzível');
  }

  const translatedText = await requestOpenClawChatCompletion([
    {
      role: 'system',
      content: 'Você é um assistente de tradução profissional。Traduza completamente as informações em inglês fornecidas pelos usuários para o chinês simplificado，Preservar a estrutura e os parágrafos originais，Não resuma mais，não explique，Não omita conteúdo。',
    },
    {
      role: 'user',
      content: `Traduza o texto completo das seguintes informações para chinês simplificado。\n\ntítulo：${article.title}\n\ntexto：\n${sourceText}`,
    },
...
export async function summarizeReaderArticle(readerDir: string, articleId: string) {
  ...
  if (!aiSummary || aiSummary.length === 0) {
    throw new Error('AI Nenhum resumo disponível foi retornado');
  }
