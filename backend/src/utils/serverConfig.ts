...
function buildDefaultPaths(): PathConfig {
  ...
  return {
    downloadsDir: path.join(home, 'download'),
    musicDownloadsDir: path.join(home, 'música'),
    localMusicDir: path.join(home, 'música'),
    notesDir: path.join(home, 'documento', 'Notas aleatórias'),
    readerDir: path.join(home, 'documento', 'RSSInformação'),
    stockAnalysisDir: path.join(home, 'documento', 'AIAnálise de negociação de ações'),
    videoDownloadsDir: path.join(home, 'vídeo'),
  };
}
