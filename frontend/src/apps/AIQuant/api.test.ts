...
/** A compilação contém headers de mock Response */
...
test('confirmStockAnalysisSignal posts payload', async () => {
  ...
  try {
    await confirmStockAnalysisSignal('signal-1', { quantity: 200, weight: 0.2, note: 'Confirmação de teste' })
    assert.match(requestBody, /200/)
    assert.match(requestBody, /0\.2/)
    assert.match(requestBody, /Confirmação de teste/)
  } finally {
    global.fetch = originalFetch
    Object.defineProperty(globalThis, 'window', { value: originalWindow, configurable: true })
  }
