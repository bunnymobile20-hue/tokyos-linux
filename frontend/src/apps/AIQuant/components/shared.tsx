...
export function LoadingState() {
  return <div className="h-full flex items-center justify-center text-slate-500">Inicializando A Dados de ações e sinais de hoje...</div>
}
    ...
    <div className="h-full flex items-center justify-center">
      <div className="max-w-lg rounded-2xl border border-red-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-3 text-red-600 mb-3">
          ...
          <div className="font-bold">AI Falha na inicialização da negociação de ações</div>
        </div>
        <p className="text-sm text-slate-600 leading-relaxed">{error}</p>
        <button onClick={onRetry} className="mt-4 px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Tente novamente</button>
      </div>
    </div>
  )
}
...
export function StatusBanner({ overview }: { overview: StockAnalysisOverview }) {
  ...
  return (
    <div className={`mx-2 mb-2 rounded-xl border px-3 py-2.5 text-xs leading-relaxed ${toneClass}`}>
      ...
      {systemStatus.latestSignalDate ? <p>Instantâneo: {systemStatus.latestSignalDate}</p> : null}
      ...
      {systemStatus.lastError ? <p className="mt-1 truncate" title={systemStatus.lastError}>erro: {systemStatus.lastError}</p> : null}
      {systemStatus.lastSuccessAt ? <p className="mt-1 text-[10px] opacity-75">sucesso: {new Date(systemStatus.lastSuccessAt).toLocaleString('zh-CN')}</p> : null}
    </div>
  )
}
        ...
        {items.length > 0 ? items.map((item) => (
          <div key={`${item.title}-${item.code ?? 'none'}`} className="rounded-xl bg-white/70 border border-white/60 p-3">
            <div className="flex items-center justify-between gap-4">
              ...
              {typeof item.score === 'number' ? <div className="text-xs text-slate-500">Pontuação abrangente {item.score}</div> : null}
            </div>
