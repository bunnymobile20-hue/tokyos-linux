...
export default function Dashboard() {
  ...
  const [testStatus, setTestStatus] = useState('Clique para iniciar o teste de velocidade')
  ...
  const runSpeedTest = async () => {
    ...
    try {
      // 1. Ping
      setTestStatus('Ping Medição de velocidade em andamento...');
      const pings = [];
      for(let i=0; i<3; i++) {
      ...
      // 2. Download (10MB)
      setTestStatus('Teste de velocidade de download em andamento...');
      const dlSizeMb = 10;
      ...
      // 3. Upload (5MB)
      setTestStatus('Teste de velocidade de upload...');
      const ulSizeMb = 5;
      ...
      setTestStatus('Teste de velocidade concluído');
    } catch (err) {
      setTestStatus('O teste de velocidade falhou');
      console.error(err);
    } finally {
      setIsTesting(false);
    }
  ...
  if (loading || !stats) {
    return (
      <div className="flex items-center justify-center h-full text-slate-500">
        <ArrowPathIcon className="w-6 h-6 animate-spin mr-2" /> Lendo o status do sistema...
      </div>
    )
  }
  ...
  const formatUptime = (seconds: number) => {
    if (!seconds) return 'apenas'
    const d = Math.floor(seconds / (3600 * 24))
    const h = Math.floor((seconds % (3600 * 24)) / 3600)
    const m = Math.floor((seconds % 3600) / 60)
    if (d > 0) return `${d}céu ${h}Hora ${m}minuto`
    if (h > 0) return `${h}Hora ${m}minuto`
    return `${m}minuto`
  }
          ...
          <div className="bg-white/50 backdrop-blur-md rounded-2xl p-6 border border-white/40 shadow-sm flex flex-col justify-between">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center text-slate-700">
                ...
                <h3 className="font-semibold">Velocidade da rede em tempo real</h3>
              </div>
            </div>
        ...
        <div className="bg-white/50 backdrop-blur-md rounded-2xl p-6 border border-white/40 shadow-sm flex flex-col justify-center">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center text-slate-700">
              ...
              <h3 className="font-semibold">Tempo de execução</h3>
            </div>
          </div>
        ...
        <div className="bg-white/50 backdrop-blur-md rounded-2xl p-6 border border-white/40 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center text-slate-700">
              ...
              <h3 className="font-semibold">Teste de velocidade de conexão</h3>
            </div>
            <button 
            ...
            >
              {isTesting ? 'Medição de velocidade em andamento...' : 'Iniciar teste de velocidade'}
            </button>
          </div>
          
          <div className="grid grid-cols-3 gap-2 mt-2">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 mb-1">Atraso (Ping)</span>
            ...
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 mb-1">download</span>
              <div className="flex items-baseline space-x-1">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 mb-1">carregar</span>
              <div className="flex items-baseline space-x-1">
