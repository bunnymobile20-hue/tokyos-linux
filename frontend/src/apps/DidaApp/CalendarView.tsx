...
export default function CalendarView() {
  ...
  if (state.appStatus === 'loading') {
    return <div className="flex-1 bg-white border-r border-slate-200 flex items-center justify-center text-slate-500">Sincronizando dados de tick...</div>;
  }

  if (state.appStatus === 'unauthorized') {
    return <div className="flex-1 bg-white border-r border-slate-200 flex items-center justify-center text-slate-500">Conta TickTick não autorizada. Acesse as configurações do sistema para concluir a autorização.</div>;
  }

  if (state.appStatus === 'error') {
    return <div className="flex-1 bg-white border-r border-slate-200 flex items-center justify-center text-red-500">Falha na sincronização：{state.appError || 'erro desconhecido'}</div>;
  }
    ...
    <div className="flex-1 bg-slate-50 flex flex-col border-r border-slate-200 min-w-[500px]">
      <div className="h-14 flex items-center justify-between px-6 border-b border-slate-200 bg-white shadow-sm z-10">
        <div className="flex items-center space-x-4">
          <h2 className="text-xl font-bold text-slate-800">{year}Ano {month + 1}lua</h2>
          <div className="flex items-center space-x-1">
            ...
            <button onClick={jumpToday} className="px-2.5 py-1 text-xs font-semibold rounded-md border border-slate-200 hover:bg-slate-50 text-slate-600 transition-colors">hoje</button>
      ...
      <div className="flex-1 flex flex-col p-4 overflow-hidden">
        <div className="grid grid-cols-7 mb-2">
          {['um', 'dois', 'três', 'Quatro', 'cinco', 'seis', 'dia'].map((dayLabel) => (
