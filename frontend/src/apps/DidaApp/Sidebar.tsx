...
export default function Sidebar() {
  ...
  const onRenameProject = async (projectId: string, currentName: string) => {
    const next = window.prompt('Renomear lista', currentName);
    if (!next || next.trim() === currentName) return;
    await renameProject(projectId, next.trim());
  };

  const onDeleteProject = async (projectId: string, projectName: string) => {
    const confirmed = window.confirm(`Confirmar lista de exclusão「${projectName}」？As tarefas da lista serão recicladas para a caixa de coleta。`);
    if (!confirmed) return;
  ...
  return (
    <div className="w-64 border-r border-slate-200 bg-[#f8fafc] flex flex-col select-none">
      <div className="h-14 flex items-center px-4 border-b border-transparent">
        <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-pink-400 to-rose-400 flex items-center justify-center text-white text-xs font-bold shadow-sm">BD</div>
        <span className="ml-3 font-semibold text-[14px] text-slate-800">Bunny Dreams</span>
      </div>

      <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
        <div className="mb-4 space-y-0.5">
          <NavItem icon={Inbox} label="caixa de coleta" mode="inbox" color="#3b82f6" currentMode={state.currentView.mode} currentId={state.currentView.id} onClick={handleNav} />
          <NavItem icon={Calendar} label="hoje" mode="today" color="#eab308" currentMode={state.currentView.mode} currentId={state.currentView.id} onClick={handleNav} />
          <NavItem icon={CalendarDays} label="Últimos 7 dias" mode="next7" color="#a855f7" currentMode={state.currentView.mode} currentId={state.currentView.id} onClick={handleNav} />
          <NavItem icon={CalendarRange} label="calendário" mode="calendar" color="#ec4899" currentMode={state.currentView.mode} currentId={state.currentView.id} onClick={handleNav} />
        </div>

        <div className="mb-4">
          <div className="px-3 flex items-center justify-between group mb-1 text-slate-400 hover:text-slate-600 cursor-pointer">
            <span className="text-xs font-bold tracking-wider">Lista de verificação</span>
          ...
          {showProjectInput ? (
            <div className="px-3 mb-2">
              <input
                ...
                autoFocus
                placeholder="Digite o nome da lista e pressione Enter para criar"
        ...
        <div className="mb-4">
          <div className="px-3 flex items-center justify-between group mb-1 text-slate-400 hover:text-slate-600 cursor-pointer">
            <span className="text-xs font-bold tracking-wider">Rótulo</span>
          </div>
