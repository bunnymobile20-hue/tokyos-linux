...
export default function TaskDatePicker({ task, onSave, onClose }: TaskDatePickerProps) {
  ...
  const quickPicks = [
    { label: 'hoje', onClick: () => { setSelectedDate(new Date()); } },
    { label: 'amanhã', onClick: () => { const d = new Date(); d.setDate(d.getDate() + 1); setSelectedDate(d); } },
    { label: 'próxima semana', onClick: () => { const d = new Date(); d.setDate(d.getDate() + 7); setSelectedDate(d); } },
    { label: 'próximo mês', onClick: () => { const d = new Date(); d.setMonth(d.getMonth() + 1); setSelectedDate(d); } },
  ];
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
        <div className="flex space-x-1">
          ...
          <span className="text-sm font-bold flex items-center justify-center min-w-[80px]">{year}Ano {month + 1}lua</span>
      ...
      <div className="p-4">
        <div className="grid grid-cols-7 mb-2">
          {['um', 'dois', 'três', 'Quatro', 'cinco', 'seis', 'dia'].map(d => (
      ...
      <div className="bg-slate-50/80 px-4 py-3 border-t border-slate-100 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <option value="0">na hora certa</option>
            <option value="-5m">antecipadamente 5 minuto</option>
            <option value="-30m">antecipadamente 30 minuto</option>
            <option value="-1h">antecipadamente 1 Hora</option>
            <option value="-1d">antecipadamente 1 céu</option>
          </select>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <option value="none">Não repetido</option>
