...
function repeatLabel(value?: string) {
  if (!value) return 'Não repetido';
  if (value === 'daily') return 'diariamente';
  if (value === 'weekly') return 'semanalmente';
  if (value === 'monthly') return 'por mês';
  if (value === 'yearly') return 'cada ano';
  return 'Repetição personalizada';
}

function reminderLabel(value?: string) {
  if (!value) return 'Nenhum lembrete';
  if (value === '0') return 'na hora certa';
  if (value === '-5m') return 'antecipadamente5minuto';
  if (value === '-30m') return 'antecipadamente30minuto';
  if (value === '-1h') return 'antecipadamente1Hora';
  if (value === '-1d') return 'antecipadamente1céu';
...
export default function TaskDetail() {
  ...
  if (!task) {
    return (
      <div className="w-80 bg-[#fbfbfb] border-l border-slate-200 flex flex-col justify-center items-center text-slate-400 select-none">
        ...
        <span className="font-medium text-sm">Clique na tarefa para ver detalhes</span>
        <span className="text-xs opacity-80 mt-2">Aqui você pode editar o título da tarefa、Descrição e propriedades</span>
      </div>
    );
  }
  ...
  const updateTask = async (updates: Partial<Task>) => {
    ...
    try {
      ...
      setSaveError(errorObj instanceof Error ? errorObj.message : 'Falha ao salvar');
    }
  };
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      setSaveError(errorObj instanceof Error ? errorObj.message : 'Falha na exclusão');
    }
  };
  ...
  const onToggleStatus = async () => {
    ...
    try {
      ...
      setSaveError(errorObj instanceof Error ? errorObj.message : 'Falha na atualização de status');
    }
  };
    ...
    <div className="w-[360px] bg-white border-l border-slate-200 flex flex-col shadow-[-10px_0_20px_-10px_rgba(0,0,0,0.05)] z-20 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar">
        ...
        <div className="flex items-center space-x-2 text-sm text-slate-600 mb-6 relative">
          ...
          {task.dueDate ? (
            <div className={`flex items-center px-3 py-1.5 rounded cursor-pointer transition-colors ${!task.isAllDay ? 'bg-blue-50 text-blue-600 font-medium hover:bg-blue-100' : 'hover:bg-slate-100 text-slate-500'}`} onClick={() => setShowDatePicker(true)}>
              ...
              {task.isAllDay ? 'o dia todo' : (parseTaskDate(task.dueDate)?.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }) || 'Anomalia de tempo')}
            </div>
          ) : null}
        ...
        <div className="mb-4 flex items-center gap-2 text-xs text-slate-500">
          <span className="px-2 py-1 bg-slate-100 rounded">lembrar: {reminderLabel(task.reminder)}</span>
          <span className="px-2 py-1 bg-slate-100 rounded">repita: {repeatLabel(task.repeat)}</span>
        </div>
      ...
      <div className="h-12 border-t border-slate-100 bg-[#fbfbfb] flex items-center justify-between px-4 text-xs font-medium flex-shrink-0">
        <div className="flex space-x-2">
          <div className="flex items-center text-slate-500 px-2 py-1.5 rounded">
            ...
            {project?.name || 'caixa de coleta'}
          </div>

          <div className="flex items-center text-slate-500 px-2 py-1.5 rounded">
            <Tag className="w-3.5 h-3.5 mr-1.5 opacity-70" />
            {task.tags.length > 0 ? task.tags.join(', ') : 'Rótulo'}
          </div>
        </div>

        <div className="text-[10px] text-slate-400">{state.isSyncing ? 'Sincronizando...' : 'Sincronizado'}</div>
      </div>
    </div>
  );
}
