...
function formatNext7GroupLabel(date: Date, todayStart: Date) {
  ...
  if (date.toDateString() === todayStart.toDateString()) return 'hoje';
...
export default function TaskList() {
  ...
  const currentViewTitle = () => {
    switch (state.currentView.mode) {
      case 'inbox': return 'caixa de coleta';
      case 'today': return 'hoje';
      case 'next7': return 'Últimos 7 dias';
      case 'project': return state.projects.find((project) => project.id === state.currentView.id)?.name || 'lista desconhecida';
  ...
  const inboxGroups =
    state.currentView.mode === 'inbox'
      ? (() => {
          ...
          return [
            { key: 'today', label: 'hoje', tasks: groups.today, tone: 'text-blue-500' },
            { key: 'tomorrow', label: 'amanhã', tasks: groups.tomorrow, tone: 'text-cyan-500' },
            { key: 'next7', label: 'Próximo7céu', tasks: groups.next7, tone: 'text-indigo-500' },
            { key: 'farther', label: 'mais', tasks: groups.farther, tone: 'text-slate-500' },
            { key: 'noDate', label: 'sem data', tasks: groups.noDate, tone: 'text-slate-500' },
          ];
        })()
      : [];
  ...
  const handleAddTask = async (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCreateError(errorObj instanceof Error ? errorObj.message : 'Falha ao criar tarefa');
    }
  };
  ...
  const onBulkComplete = async () => {
    try {
      ...
      setCreateError(errorObj instanceof Error ? errorObj.message : 'Falha na conclusão do lote');
    }
  };

  const onBulkDelete = async () => {
    const confirmed = window.confirm(`Confirme para excluir selecionado ${selectedTasks.length} tarefas？`);
    if (!confirmed) return;
    try {
      await bulkDeleteTasks(selectedTasks);
      clearSelection();
    } catch (errorObj) {
      setCreateError(errorObj instanceof Error ? errorObj.message : 'Falha na exclusão do lote');
    }
  };
  ...
  const onBulkMove = async () => {
    ...
    const targetName = window.prompt(`Insira o nome da lista de destino:\n${options.map((project) => `- ${project.name}`).join('\n')}`);
    if (!targetName) return;

    const target = options.find((project) => project.name === targetName.trim());
    if (!target) {
      setCreateError('A lista de alvos não existe');
      return;
    }
    ...
    try {
      ...
      setCreateError(errorObj instanceof Error ? errorObj.message : 'Falha na movimentação do lote');
    }
  };
  ...
  const onDrop = (targetTaskId: string) => async (event: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      setCreateError(errorObj instanceof Error ? errorObj.message : 'Falha na classificação');
    } finally {
      setDragTaskId(null);
    }
  };
  ...
  const onRefresh = async () => {
    ...
    try {
      ...
      setCreateError(errorObj instanceof Error ? errorObj.message : 'Falha na atualização');
    }
  };
  ...
  const renderInputPreview = () => {
    if (!newTaskTitle.trim()) {
      return <span className="text-slate-400">{`Adicionar tarefa a「${currentViewTitle()}」，Pressione Enter para salvar`}</span>;
    }
  ...
  if (state.appStatus === 'loading') {
    return <div className="flex-1 border-r border-slate-200 bg-white flex items-center justify-center text-slate-500">Sincronizando dados de tick...</div>;
  }
  if (state.appStatus === 'unauthorized') {
    return <div className="flex-1 border-r border-slate-200 bg-white flex items-center justify-center text-slate-500">Conta TickTick não autorizada. Acesse as configurações do sistema para concluir a autorização.</div>;
  }
  if (state.appStatus === 'error') {
    return <div className="flex-1 border-r border-slate-200 bg-white flex items-center justify-center text-red-500">Falha na sincronização：{state.appError || 'erro desconhecido'}</div>;
  }
    ...
    <div className="flex-1 border-r border-slate-200 bg-white flex flex-col min-w-[300px]">
      <div className="h-14 flex items-center justify-between px-6 border-b border-transparent sticky top-0 bg-white z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-slate-400 text-sm font-medium">{activeTasks.length} item</div>
        </div>
      </div>
      ...
      <div className="px-4 pt-1 pb-2 bg-white border-b border-slate-100 flex items-center justify-between text-xs text-slate-500">
        <button className="hover:text-slate-700" onClick={() => { setSelectionMode((prev) => !prev); setSelectedTaskIds([]); }}>
          {selectionMode ? 'Sair da seleção múltipla' : 'Múltipla escolha'}
        </button>
        {selectionMode ? <span>Selecionado {selectedTaskIds.length}</span> : <span>Arraste e solte para classificar</span>}
      </div>
      ...
      {selectionMode && selectedTaskIds.length > 0 ? (
        <div className="px-4 py-2 border-b border-slate-100 bg-slate-50 flex items-center gap-2 text-xs">
          <button onClick={() => void onBulkComplete()} className="px-2 py-1 rounded bg-emerald-100 text-emerald-700 hover:bg-emerald-200 flex items-center gap-1"><CheckCheck className="w-3 h-3" />Terminar</button>
          <button onClick={() => void onBulkMove()} className="px-2 py-1 rounded bg-blue-100 text-blue-700 hover:bg-blue-200 flex items-center gap-1"><ArrowRightLeft className="w-3 h-3" />mover</button>
          <button onClick={() => void onBulkDelete()} className="px-2 py-1 rounded bg-red-100 text-red-700 hover:bg-red-200 flex items-center gap-1"><Trash2 className="w-3 h-3" />excluir</button>
          <button onClick={clearSelection} className="ml-auto px-2 py-1 rounded bg-white border border-slate-200">Cancelar</button>
        </div>
      ) : null}
        ...
        <div className="space-y-1">
          {activeTasks.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-slate-400">
              ...
              <span className="text-sm font-medium">Não há tarefas na visualização atual</span>
              <span className="text-xs mt-1 opacity-70">Clique na caixa de entrada acima para adicionar uma nova tarefa</span>
            </div>
          ) : state.currentView.mode === 'inbox' ? (
            <>
              {inboxGroups.map((group) => (
                <div key={group.key}>
                  ...
                  {group.tasks.length === 0 ? (
                    <div className="px-6 py-1 text-[11px] text-slate-300">Nenhuma tarefa ainda</div>
                  ) : (
            ...
            <>
              {overdueTasks.length > 0 ? (
                <>
                  <div className="px-4 pt-1 text-[11px] font-semibold text-red-500">Atrasado</div>
              ...
              {todayTasks.length > 0 ? (
                <>
                  <div className="px-4 pt-2 text-[11px] font-semibold text-blue-500">hoje</div>
                  {todayTasks.map((task) => (
                    <TaskItem
                      key={task.id}
                      task={task}
          ...
          {completedTasks.length > 0 ? (
            <div className="mx-2 mt-4 border-t border-slate-100 pt-2">
              <button onClick={() => setShowCompleted((prev) => !prev)} className="text-xs text-slate-500 hover:text-slate-700 px-2 py-1">
                {showCompleted ? 'esconder' : 'mostrar'}Concluído ({completedTasks.length})
              </button>
              {showCompleted ? (
