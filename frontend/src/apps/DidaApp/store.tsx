...
const initialState: StoreState = {
  tasks: [],
  projects: [{ id: 'inbox', name: 'caixa de coleta', color: '#3b82f6', isSystem: true }],
  tags: [],
...
function normalizeProjects(projects: Project[]): Project[] {
  ...
  return [{ id: 'inbox', name: 'caixa de coleta', color: '#3b82f6', isSystem: true }, ...projects];
}
...
function reducer(state: StoreState, action: Action): StoreState {
  switch (action.type) {
    ...
    case 'LOAD_UNAUTHORIZED':
      return { ...state, appStatus: 'unauthorized', appError: 'O tick não está autorizado，Acesse as configurações do sistema para concluir a autorização' };
    case 'LOAD_ERROR':
...
function notifyDidaFailure(title: string, message: string, options?: { dedupeKey?: string; batchKey?: string }) {
  return notifyDida({
    ...
    batchTitle: title,
    batchMessageBuilder: (count, latestMessage) => `${latestMessage}${count > 1 ? ` (Um total de ${count} tarefas)` : ''}`,
  })
}
...
function useAppStoreInternal() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (!status.success || !status.connected) {
        ...
        void notifyDida({
          title: 'O tick não está autorizado',
          message: 'Por favor, vá para as configurações do sistema para concluir a autorização do tick',
          level: 'warning',
      ...
      void notifyDidaFailure('Falha na sincronização do tick', message, { dedupeKey: `sync:${message}` })
    } finally {
  ...
  const createTask = async (draft: Partial<Task> & { title: string; projectId: string }) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha ao criar tarefa', extractErrorMessage(errorObj), { batchKey: 'task-create-failed' })
      throw errorObj;
    } finally {
  ...
  const saveTask = async (task: Task) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha ao salvar a tarefa', extractErrorMessage(errorObj), { batchKey: 'task-save-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const removeTask = async (task: Task) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha ao excluir tarefa', extractErrorMessage(errorObj), { batchKey: 'task-delete-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const toggleTaskStatus = async (task: Task) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha ao atualizar o status da tarefa', extractErrorMessage(errorObj), { batchKey: 'task-status-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const createProject = async (name: string, color: string) => {
    ...
    try {
      ...
      void notifyDida({ title: 'Lista criada', message: created.name, level: 'success' })
    } catch (errorObj) {
      void notifyDidaFailure('Falha ao criar o manifesto', extractErrorMessage(errorObj), { batchKey: 'project-create-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const renameProject = async (projectId: string, name: string) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha ao renomear lista', extractErrorMessage(errorObj), { batchKey: 'project-rename-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const deleteProject = async (projectId: string) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha ao excluir lista', extractErrorMessage(errorObj), { batchKey: 'project-delete-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const bulkCompleteTasks = async (tasks: Task[]) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha na atualização do status do lote', extractErrorMessage(errorObj), { batchKey: 'bulk-status-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const bulkDeleteTasks = async (tasks: Task[]) => {
    ...
    try {
      ...
      void notifyDidaFailure('Falha na exclusão do lote', extractErrorMessage(errorObj), { batchKey: 'bulk-delete-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const moveTasksToProject = async (tasks: Task[], projectId: string) => {
    ...
    try {
      ...
      for (const snapshot of snapshots) dispatch({ type: 'SYNC_REVERT', payload: snapshot });
      void notifyDidaFailure('Falha na movimentação do lote', extractErrorMessage(errorObj), { batchKey: 'bulk-move-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const renameTag = async (from: string, to: string) => {
    ...
    try {
      ...
      for (const snapshot of snapshots) dispatch({ type: 'SYNC_REVERT', payload: snapshot });
      void notifyDidaFailure('Falha ao renomear marcador', extractErrorMessage(errorObj), { batchKey: 'tag-rename-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
  ...
  const deleteTag = async (tag: string) => {
    ...
    try {
      ...
      for (const snapshot of snapshots) dispatch({ type: 'SYNC_REVERT', payload: snapshot });
      void notifyDidaFailure('Falha ao excluir tag', extractErrorMessage(errorObj), { batchKey: 'tag-delete-failed' })
      throw errorObj;
    } finally {
      dispatch({ type: 'SYNC_FINISH' });
    }
