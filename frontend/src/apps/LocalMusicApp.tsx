...
export default function LocalMusicApp() {
  ...
  const loadLibrary = async () => {
    ...
    try {
      ...
      if (data.success) {
        setSongs(data.data)
        if (data.needsScan) {
          showToast('O diretório atual ainda não foi verificado，Por favor clique“Diretório de verificação”Criar biblioteca de música', 'info')
        }
      }
    } catch (e) {
      console.error(e)
      showToast('Falha ao carregar a biblioteca de música，Verifique o status do back-end', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const scanLibrary = async () => {
    if (!musicDir) {
      showToast('Por favor configure primeiro o diretório de música local', 'error')
      return
    }
    setLoading(true)
    showToast('Iniciar a digitalização do diretório，Por favor, aguarde...', 'info')
    try {
      ...
      if (data.success) {
        ...
        if (data.data.length > 0) {
          showToast(`Verificação concluída，Encontrado no total ${data.data.length} canção`, 'success')
        } else {
          showToast(`Verificação concluída，Nenhum arquivo de áudio compatível foi encontrado neste diretório`, 'info')
        }
      } else {
        showToast(data.error || 'Falha na verificação', 'error')
      }
    } catch (e) {
      console.error(e)
      showToast('Falha na solicitação de rede', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleMusicDirChange = async (nextDir: string) => {
    try {
      ...
      showToast('Falha ao salvar o diretório', 'error')
    }
  }
    ...
    const handleDirUpdated = () => {
      fetchServerPaths()
        .then((paths) => {
          ...
          setIsPlaying(false)
          pendingSeekRef.current = null
          showToast('Configuração de diretório atualizada', 'success')
        })
  ...
  const playSong = (song: LocalTrack) => {
    ...
    if (audioRef.current) {
      ...
      playCurrentAudio('Falha na reprodução do arquivo local，Verifique se o arquivo de áudio está danificado')
    }
    fetchLyrics(song.id)
  }
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      playCurrentAudio('Falha ao continuar jogando，Clique em reproduzir novamente manualmente')
    }
  }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 text-slate-800 relative overflow-hidden">
      ...
      <audio 
        ...
        onError={() => {
          setIsPlaying(false)
          showToast('Falha ao carregar áudio，Digitalize novamente ou verifique o arquivo', 'error')
        }}
      ...
      <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200 z-10 relative">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-lg font-bold text-slate-800">música local Pro</h2>
        </div>
        <div className="flex items-center space-x-3">
          <button 
            onClick={scanLibrary}
          ...
          >
            ...
            Diretório de verificação
          </button>
          <DirSetting 
            label="diretório de música local" 
            value={musicDir} 
            onChange={(nextDir) => { void handleMusicDirChange(nextDir) }} 
            description="Por favor insira o caminho absoluto para armazenar a música。O backend irá verificar recursivamente os arquivos de áudio neste diretório。"
          />
        </div>
      </div>
      ...
      {warmupStatus && warmupStatus.scannedDir === musicDir && (warmupStatus.running || warmupStatus.updated > 0) && (
        <div className="px-6 py-3 bg-emerald-50 border-b border-emerald-100 flex items-center justify-between text-xs text-emerald-700">
          <div className="flex items-center space-x-3 min-w-0">
            <span className="font-semibold whitespace-nowrap">Conclusão de metadados</span>
            <span className="whitespace-nowrap">{warmupStatus.completed}/{warmupStatus.total}</span>
            <span className="whitespace-nowrap">atualizado {warmupStatus.updated} cabeça</span>
            {warmupStatus.currentTrack && (
              <span className="truncate">atual: {warmupStatus.currentTrack}</span>
            )}
          </div>
      ...
      <div className="flex-1 overflow-auto bg-slate-50 p-6 pb-28 relative z-0">
        {loading ? (
          <div className="flex items-center justify-center h-32 text-slate-400">Verificando e analisando arquivos de música，Por favor, aguarde...</div>
        ) : songs.length === 0 ? (
           <div className="flex flex-col items-center justify-center h-48 text-slate-400">
             <ViewColumnsIcon className="w-12 h-12 mb-4 opacity-50" />
             <p>A biblioteca de música local está vazia，Configure o diretório e clique em「Diretório de verificação」</p>
           </div>
        ) : (
          <div className="w-full max-w-5xl mx-auto">
            <div className="grid grid-cols-12 gap-4 px-4 py-2 border-b border-slate-200 text-xs font-medium text-slate-400 mb-2">
              ...
              <div className="col-span-5">título da música</div>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-20 ${showImmersive ? 'bg-transparent text-white' : 'bg-white/95 text-slate-800 backdrop-blur-xl border-t border-slate-200'} flex items-center justify-between px-6 shadow-[0_-4px_20px_rgba(0,0,0,0.04)] z-50 transition-colors duration-500`}>
        ...
        >
          ...
          <div className="truncate">
            <div className={`text-sm font-bold truncate ${showImmersive ? 'text-white' : 'text-slate-800'}`} title={currentSong?.name}>{currentSong?.name || 'música local'}</div>
            <div className={`text-xs truncate ${showImmersive ? 'text-white/60' : 'text-slate-500'}`}>{currentSong?.artist || 'O que você quer ouvir'}</div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center flex-1 max-w-lg">
          <div className="flex items-center space-x-6 mb-1.5">
            {/* Play Mode Toggle */}
            <button onClick={togglePlayMode} className={`${showImmersive ? 'text-white/60 hover:text-white' : 'text-slate-400 hover:text-[#31c27c]'} transition-colors`} title={playMode === 'sequence' ? 'Jogue sequencialmente' : playMode === 'random' ? 'embaralhar' : 'laço único'}>
              {playMode === 'sequence' ? (
                ...
                <div className="flex items-center text-[10px] font-bold"><ArrowPathIcon className="w-4 h-4 mr-0.5 text-[#31c27c]" /> um</div>
              )}
            </button>
