...
export default function ServiceMonitor() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full text-slate-500">
        <ArrowPathIcon className="w-6 h-6 animate-spin mr-2" /> Conexão da sonda em andamento...
      </div>
    )
  }
  ...
  const getStatusLabel = (service: ServiceStatus) => {
    if (service.isRunning) {
      return { text: 'Correndo', className: 'text-green-600 bg-green-500/10', dot: 'bg-green-500 animate-pulse' }
    }

    if (service.status === 'activating') {
      return { text: 'Começando', className: 'text-amber-600 bg-amber-500/10', dot: 'bg-amber-500 animate-pulse' }
    }

    return { text: 'Não está em execução', className: 'text-red-600 bg-red-500/10', dot: 'bg-red-500' }
  }
  ...
  const formatWatchdogTimestamp = (timestamp?: string) => {
    if (!timestamp) {
      return 'Aguardando a primeira inspeção'
    }
        ...
        <div className="flex flex-col items-center justify-center min-h-32 bg-white/40 rounded-xl border border-white/20 px-4 py-4 text-center">
          {backup?.error ? (
            ...
            <>
              ...
              <div className="text-sm text-slate-500 mb-1">backup mais recente</div>
              ...
              {options.showCount && typeof extendedBackup?.count === 'number' && (
                <div className="mt-2 text-xs text-slate-500">comum {extendedBackup.count} backup</div>
              )}
            ...
            <div className="text-slate-500 text-sm">{backup?.message || 'Nenhum registro de backup ainda'}</div>
          )}
        </div>
      </div>
    )
      ...
      if (!data.latest) {
        return (
          <div className="rounded-xl border border-slate-200 bg-white/50 px-3 py-3">
            <div className="text-sm font-semibold text-slate-800">{label}</div>
            <div className="mt-1 text-xs text-slate-500">{data.message || 'Nenhum registro de backup ainda'}</div>
            <div className="mt-1 text-[11px] text-slate-400 break-all">{data.directory}</div>
          </div>
        )
      }
    ...
    return (
      <div className="break-inside-avoid mb-4 bg-white/50 backdrop-blur-md rounded-2xl p-4 border border-white/40 shadow-sm">
        <div className="flex items-center text-slate-700 mb-3">
          ...
          <h3 className="font-semibold text-lg">Backup em nível de aplicativo</h3>
        </div>
        ...
        {!backup ? (
          <div className="flex flex-col items-center justify-center min-h-32 bg-white/40 rounded-xl border border-white/20 px-4 text-center text-slate-500 text-sm">
            Ainda não há informações de backup
          </div>
        ) : (
          <div className="space-y-3">
              <div className={`rounded-xl px-3 py-2 text-[11px] ${getSyncMeta().className}`}>
                <div className="font-semibold">Status de sincronização：{backup.syncMessage}</div>
              </div>
              <div className="rounded-xl border border-white/20 bg-white/40 px-3 py-2 text-[11px] text-slate-500">
                <div className="font-semibold text-slate-700">ClawBackUp / OpenClaw Backup do espaço de trabalho</div>
                <div>diretório raiz：<span className="break-all">{backup.rootDirectory}</span></div>
                <div className="mt-1">índice de versão：{backup.hasIndexFile ? 'Encontrado VERSIONS.md' : 'não encontrado VERSIONS.md'}</div>
                {backup.latestIndexedVersion && backup.latestIndexedStamp && (
                <div className="mt-1">Indexar versão mais recente：{backup.latestIndexedVersion}-{backup.latestIndexedStamp}</div>
              )}
            </div>
            {renderBackupRow('Backup de versão do diretório', backup.versions)}
            {renderBackupRow('Zip Compactar backup', backup.zips)}
          </div>
        )}
      </div>
    )
    ...
    return (
      <div className="break-inside-avoid mb-4 bg-white/50 backdrop-blur-md rounded-2xl p-4 border border-white/40 shadow-sm">
        <div className="flex items-center text-slate-700 mb-3">
          ...
          <h3 className="font-semibold text-lg">Backup em nível de máquina inteira</h3>
        </div>

        {!backup ? (
          <div className="flex flex-col items-center justify-center min-h-32 bg-white/40 rounded-xl border border-white/20 px-4 text-center text-slate-500 text-sm">
            Nenhum restic Fazer backup de informações
          </div>
        ) : (
          <div className="space-y-3">
            <div className={`rounded-xl px-3 py-2 text-[11px] ${getSyncMeta().className}`}>
              <div className="font-semibold">Estado de observação：{backup.syncMessage}</div>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white/50 px-3 py-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-sm font-semibold text-slate-800">local restic armazém</div>
                  {backup.localRepo.error ? (
                    ...
                    <div className="mt-1 text-xs text-slate-500">{backup.localRepo.message || 'Nenhum instantâneo ainda'}</div>
                  )}
                </div>
                <div className="shrink-0 rounded-full bg-violet-50 px-2 py-1 text-[10px] font-semibold text-violet-700">
                  {backup.localRepo.count} compartilhar
                </div>
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white/50 px-3 py-3 text-[11px] text-slate-600 space-y-2">
              <div>
                <span className="font-semibold text-slate-800">Nuvem：</span>
                <span>{backup.cloud.configured ? 'Nuvem Alibaba OSS configurado' : 'Nuvem Alibaba OSS Não configurado'}</span>
              </div>
              {backup.cloud.repository && (
                <div className="break-all text-slate-500">armazém：{backup.cloud.repository}</div>
              )}
              <div>
                <span className="font-semibold text-slate-800">Tarefas agendadas：</span>
                <span>{backup.schedule.configured ? 'configurado' : 'Não configurado'}</span>
                {backup.schedule.expression ? ` (${backup.schedule.expression})` : ''}
              </div>
              <div className="text-slate-500 break-all">diretório raiz：{backup.rootDirectory}</div>
            </div>
          </div>
        )}
      </div>
  ...
  const renderSecurityCard = (status: SecuritySurfaceStatus | null) => {
    ...
    const exposureLabel = (value: SecuritySurfaceStatus['ports'][number]['actual']) => {
      switch (value) {
        case 'local-only':
          return 'Somente esta máquina'
        case 'direct-access':
          return 'Monitoramento externo'
        default:
          return 'Não estou ouvindo'
      }
    }
    ...
    return (
      <div className="break-inside-avoid mb-4 bg-white/50 backdrop-blur-md rounded-2xl p-4 border border-white/40 shadow-sm">
        <div className="flex items-center text-slate-700 mb-3">
          ...
          <h3 className="font-semibold text-lg">estado seguro</h3>
        </div>

        {!status ? (
          <div className="flex flex-col items-center justify-center min-h-32 bg-white/40 rounded-xl border border-white/20 px-4 text-center text-slate-500 text-sm">
            Ainda não há informações de status de segurança
          </div>
        ) : (
          <div className="space-y-3">
              ...
              {status.ports.map((port) => (
                <div key={port.port} className="flex items-center justify-between gap-3 text-xs">
                  <div>
                    ...
                    <div className="text-slate-400">:{port.port} / esperado {port.expected === 'local-only' ? 'Somente esta máquina' : 'Permitir conexão direta'}</div>
                  </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white/50 px-3 py-3 text-xs text-slate-600 space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-slate-800">AList Senha de administrador</span>
                <span className={status.credentials.alistAdmin.weak ? 'text-amber-700' : 'text-emerald-700'}>{status.credentials.alistAdmin.weak ? 'senha fraca' : 'substituído'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="font-semibold text-slate-800">aria2 RPC Secret</span>
                <span className={status.credentials.aria2Secret.weak ? 'text-amber-700' : 'text-emerald-700'}>{status.credentials.aria2Secret.weak ? 'chave fraca' : 'substituído'}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          {service.kind === 'core' && service.health && (
            <div className="text-[10px] text-slate-400 mt-1.5 flex items-center gap-2 flex-wrap">
              <span className={`px-1.5 py-0.5 rounded-sm font-semibold ${healthLabel.className}`}>saudável: {healthLabel.text}</span>
              <span className={`px-1.5 py-0.5 rounded-sm font-semibold ${riskLabel.className}`}>risco: {riskLabel.text}</span>
          ...
          {service.kind === 'watchdog' && (
            <div className="text-[10px] text-slate-400 mt-1.5 flex items-center gap-2 flex-wrap">
              <span className="bg-white/50 px-1.5 py-0.5 rounded-sm">recente: {getWatchdogResultLabel(service.watchdogStatus?.result)}</span>
              <span className={`px-1.5 py-0.5 rounded-sm font-semibold ${riskLabel.className}`}>risco: {riskLabel.text}</span>
              <span className="truncate max-w-[120px]" title={service.watchdogStatus?.message}>{service.watchdogStatus?.message ?? 'Aguardando inspeção'}</span>
          ...
          <div className="text-[10px] text-slate-400 mt-1.5">Ação recomendada：{actionSuggestion}</div>
        </div>
      ...
      <div className="columns-1 md:columns-2 xl:columns-3 gap-6">
        ...
        <div className="break-inside-avoid mb-6 flex flex-col justify-center rounded-2xl border border-white/40 bg-white/50 px-4 py-4 shadow-sm backdrop-blur-md">
          <div className="flex items-center justify-between gap-2">
            <div className="flex flex-col items-center flex-1">
              <span className="text-[10px] text-slate-500 uppercase tracking-wider font-medium mb-1">total</span>
              <span className="text-xl font-bold text-slate-800">{summary.total}</span>
            </div>
            <div className="w-px h-8 bg-slate-200/80"></div>
            <div className="flex flex-col items-center flex-1">
              <span className="text-[10px] text-red-500 uppercase tracking-wider font-medium mb-1">alto risco</span>
              <span className="text-xl font-bold text-red-700">{summary.highRisk}</span>
            </div>
            <div className="w-px h-8 bg-slate-200/80"></div>
            <div className="flex flex-col items-center flex-1">
              <span className="text-[10px] text-amber-600 uppercase tracking-wider font-medium mb-1">Precisa de atenção</span>
              <span className="text-xl font-bold text-amber-700">{summary.warning}</span>
            </div>
            <div className="w-px h-8 bg-slate-200/80"></div>
            <div className="flex flex-col items-center flex-1">
              <span className="text-[10px] text-emerald-600 uppercase tracking-wider font-medium mb-1">normal</span>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-center text-[10px] text-slate-400">
            ...
            A investigação do sistema está em execução
          </div>
        </div>
        ...
        <div className="break-inside-avoid mb-6 bg-white/50 backdrop-blur-md rounded-2xl p-4 border border-white/40 shadow-sm">
          <div className="flex items-center text-slate-700 mb-3">
            ...
            <h3 className="font-semibold text-lg">Serviços principais</h3>
          </div>
        ...
        <div className="break-inside-avoid mb-6 bg-white/50 backdrop-blur-md rounded-2xl p-4 border border-white/40 shadow-sm">
          <div className="flex items-center text-slate-700 mb-3">
            ...
            <h3 className="font-semibold text-lg">Guarda automática</h3>
          </div>
          <div className="space-y-2">
        ...
        {renderBackupCard({ title: 'Backup em nível de sistema', accentClassName: 'text-teal-500', backup: timeshift })}
