...
export default function MusicApp({ isActive = true }: { isActive?: boolean }) {
  ...
  const handleMusicDownloadDirChange = async (nextDir: string) => {
    try {
      ...
      showToast('Falha ao salvar o diretório de músicas', 'error')
    }
  }
  ...
  const playSong = async (song: SongItem, overrideQuality?: string) => {
    ...
    reportMusicState({
      ...
      playing: false,
      title: song.title,
      artist: song.artist || 'cantor desconhecido',
      cover: song.cover || '',
      lyric: ''
    })
    void fetchSongDetail(song)
    ...
    if (localTrack && resolvedDir) {
      const started = await startPlayback(
        ...
        'local',
        'Falha na reprodução do arquivo local，Verifique se o arquivo ainda existe'
      )
      if (!started) {
        return
      }
    ...
    try {
      ...
      if (data.success && data.data.url) {
        const started = await startPlayback(
          data.data.url,
          'remote',
          'Falha na reprodução，Pode ser que o navegador esteja bloqueando a reprodução automática ou o link esteja quebrado.'
        )
        if (started) {
          fetchLyrics(song.id)
        }
      } else {
        showToast('Não foi possível obter o link de reprodução para esta qualidade de som，Pode exigir restrições de adesão ou direitos autorais', 'error')
        setIsPlaying(false)
      }
    } catch (e) {
      console.error(e)
      showToast('Erro de reprodução', 'error')
      setIsPlaying(false)
    }
  }
    ...
    if (playMode === 'random') {
      ...
      if (audioRef.current) {
        ...
        audioRef.current.play().catch((error) => {
          console.error(error)
          setIsPlaying(false)
          showToast('Falha na repetição，Por favor, tente novamente mais tarde', 'error')
        })
        return
      }
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      audioRef.current.play().catch((error) => {
        console.error(error)
        setIsPlaying(false)
        showToast('Falha ao continuar jogando，Clique em reproduzir novamente manualmente', 'error')
      })
    }
  }
  ...
  useEffect(() => {
    ...
    reportMusicState({
      ...
      artist: currentSong.artist || 'cantor desconhecido',
    ...
    try {
      const res = await fetch('/api/system/music/download', {
        ...
        if (data.success && data.data?.result) {
          showToast('Empurrado para o gerenciador de downloads', 'success')
          refreshDownloadedTracks()
        } else {
          showToast('Falha no download: ' + JSON.stringify(data.data || data.error || data), 'error')
        }
    } catch (e) {
      console.error(e)
      showToast('Erro de download', 'error')
    }
  }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 text-slate-800 relative overflow-hidden">
      ...
      <audio 
        ...
        onError={() => {
          setIsPlaying(false)
          showToast('Falha ao carregar áudio，Por favor, tente novamente mais tarde', 'error')
        }}
      ...
      <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-lg font-bold text-slate-800">Música na nuvem NetEase</h2>
        </div>
        <div className="flex items-center space-x-3">
          {userInfo && (
          ...
          <DirSetting 
            label="Diretório de download de músicas" 
            value={downloadDir} 
            onChange={(nextDir) => { void handleMusicDownloadDirChange(nextDir) }} 
            description="Deixe em branco para usarAria2Diretório de download padrão global。"
          />
        </div>
      </div>
        ...
        <div className="w-56 bg-white border-r border-slate-200 p-4 flex flex-col space-y-6 overflow-y-auto">
          {!userInfo && (
            <div className="p-3 bg-rose-50 rounded-lg text-xs text-rose-600 text-center mb-2">
              Ainda não logado<br/>Por favor, vá para a área de trabalho“Configurações do sistema”fundo<br/>Use a nuvem NetEaseAPPDigitalize o código para fazer login
              <br/><br/>
              <button onClick={loadUserInfo} className="underline text-rose-500 font-bold cursor-pointer">Conectado？Clique para atualizar a interface</button>
            </div>
          )}

          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-2">Descobrir</p>
            <div 
              onClick={() => {
            ...
            >
              ...
              <span>Pesquise em toda a rede</span>
            </div>
          </div>

          {userInfo && playlists.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-2">Lista de reprodução criada</p>
          ...
          {userInfo && playlists.length > 0 && (
            <div className="space-y-1 pb-20">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-2">Lista de reprodução favorita</p>
              {playlists.filter(p => p.creator.userId !== userInfo.userId).map(p => (
                <div 
                  key={p.id}
                  onClick={() => openPlaylist(p.id)}
        ...
        <div className="flex-1 overflow-auto bg-slate-50 flex flex-col relative pb-20">
          ...
          {activeTab === 'search' && (
            <div className="p-8 border-b border-slate-200 bg-white">
              <h1 className="text-2xl font-bold text-slate-800 mb-6">Pesquise em toda a rede</h1>
              <form onSubmit={handleSearch} className="flex items-center w-full max-w-2xl">
                <div className="relative flex-1">
                  ...
                  <input 
                    ...
                    placeholder="Procure por cantor、canção、álbum..." 
              ...
              {recentSearches.length > 0 && (
                <div className="mt-4 flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-slate-400 font-medium">Pesquisas recentes</span>
          ...
          <div className="flex-1 p-6">
            {loading && songs.length === 0 && <p className="text-slate-500 mt-4">carregando...</p>}
            {!loading && !refreshing && songs.length === 0 && activeTab !== 'search' && <p className="text-slate-500 mt-4">Vazio</p>}
            {!loading && !refreshing && songs.length === 0 && activeTab === 'search' && !searchInput.trim() && <p className="text-slate-400 mt-10 text-center">Insira palavras-chave para começar a pesquisar</p>}
            
            {songs.length > 0 && (
              <div className="w-full">
                <div className="flex items-center justify-between mb-4">
                  <div className="text-sm font-bold text-slate-700">
                    {activeTab === 'search' ? 'Resultados da pesquisa' : 'Lista de reprodução'} <span className="text-slate-400 font-normal">({songs.length}cabeça)</span>
                    {refreshing ? <span className="ml-2 text-[11px] font-medium text-rose-400">Refrescante...</span> : null}
                  </div>
                  <button 
                    ...
                    >
                      ...
                      <div className={`col-span-4 font-medium flex items-center pr-4 ${isPlayingThis ? 'text-rose-600' : 'text-slate-800'}`}>
                        ...
                        {getDownloadedTrack(song) && (
                          <CheckCircleIcon className="w-4 h-4 text-green-500 ml-2 flex-shrink-0" title="Baixado，Prioridade de reprodução local" />
                        )}
                        <div className="flex items-center space-x-2 ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
                          <ArrowDownTrayIcon 
                            ...
                            title={`empurrar paraAria2download (${quality}qualidade de som)`} 
        ...
        {showImmersive && currentSong && (
          ...
          >
            ...
            <div className="h-20 flex items-center justify-between px-8 z-20">
              ...
              >
                {pureLyricMode ? 'Sair do modo limpo' : 'Modo de letras puras'}
              </button>
            </div>
                ...
                >
                  {lyrics.length === 0 ? (
                    <div className="h-full flex items-center justify-center text-white/40 text-lg">
                      música pura，Por favor, aproveite
                    </div>
                  ) : (
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-20 ${showImmersive ? 'bg-transparent text-white border-transparent' : 'bg-white/95 backdrop-blur-xl border-t border-slate-200 text-slate-800'} flex items-center justify-between px-6 shadow-[0_-4px_20px_rgba(0,0,0,0.04)] z-50 transition-colors duration-500`}>
        ...
        <div className="flex items-center space-x-4 w-64 cursor-pointer group" onClick={() => currentSong && setShowImmersive(!showImmersive)}>
          ...
          <div className="truncate">
            <div className={`text-sm font-bold truncate ${showImmersive ? 'text-white' : 'text-slate-800'}`} title={currentSong?.title}>{currentSong?.title || 'Não jogado'}</div>
            <div className={`text-xs truncate ${showImmersive ? 'text-white/60' : 'text-slate-500'}`}>{currentSong?.artist || 'Música na nuvem NetEase'}</div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center flex-1 max-w-lg">
          <div className="flex items-center space-x-6 mb-1.5">
            {/* Play Mode Toggle */}
            <button onClick={togglePlayMode} className={`${showImmersive ? 'text-white/60 hover:text-white' : 'text-slate-400 hover:text-rose-500'} transition-colors`} title={playMode === 'sequence' ? 'Jogue sequencialmente' : playMode === 'random' ? 'embaralhar' : 'laço único'}>
              {playMode === 'sequence' ? (
                <div className="flex items-center text-[10px] font-bold"><ArrowPathIcon className="w-4 h-4 mr-0.5" /> Evitar</div>
              ) : playMode === 'random' ? (
                <div className="flex items-center text-[10px] font-bold"><ArrowsRightLeftIcon className="w-4 h-4 mr-0.5" /> Seguir</div>
              ) : (
                <div className="flex items-center text-[10px] font-bold"><ArrowPathIcon className="w-4 h-4 mr-0.5 text-rose-500" /> um</div>
              )}
            </button>
            ...
            <select 
              ...
              title="Mudar a qualidade do som"
            >
              <option value="standard" className="text-slate-800 bg-white">padrão</option>
              <option value="exhigh" className="text-slate-800 bg-white">extremamente alto</option>
              <option value="lossless" className="text-slate-800 bg-white">Sem perdas</option>
        ...
        <div className={`w-64 flex justify-end items-center space-x-4 ${showImmersive ? 'text-white/60' : 'text-slate-500'}`}>
          {currentSong && (
            <ArrowDownTrayIcon 
              ...
              title="Empurre a qualidade de som atual paraAria2download" 
