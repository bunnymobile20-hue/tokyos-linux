...
export default function VideoApp() {
  ...
  const handleVideoDownloadDirChange = async (nextDir: string) => {
    try {
      ...
      alert('Falha ao salvar o diretório de vídeo')
    }
  }
  ...
  const openPlayer = (video: any) => {
    ...
    if (urls.length > 0) {
      ...
      alert('Nenhum jogável encontrado m3u8 Link')
    }
  }
  ...
  const pushToAria2 = async (e: React.MouseEvent, video: any) => {
    ...
    try {
      ...
      if (rpcData.result) alert('Empurrado para o gerenciador de downloads')
      else alert('Falha no download')
    } catch (err) {
      alert('Erro de download push')
    }
  }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 text-slate-800 relative">
      ...
      <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200">
        <div className="flex items-center space-x-6">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-bold text-slate-800">Armazém de cinema e televisão</h2>
          </div>
          <div className="flex items-center space-x-4">
            ...
            >
              <option value="">Pesquise em toda a web (Mais devagar)</option>
              {sources.map(s => (
                <option key={s.id} value={s.id}>{s.name}</option>
              ))}
            </select>
            <DirSetting 
              label="Diretório de download de vídeo" 
              value={downloadDir} 
        ...
        <form onSubmit={handleSearch} className="relative">
          ...
          <input 
            type="text" 
            placeholder="Pesquise recursos de filmes e televisão..." 
      ...
      <div className="flex-1 overflow-auto p-6 relative" onScroll={handleScroll}>
        {loading && results.length === 0 && <div className="text-slate-500 text-center mt-10">carregando...</div>}
        {!loading && results.length === 0 && search && <div className="text-slate-500 text-center mt-10">Nenhum recurso relevante encontrado</div>}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          {results.map((movie, idx) => (
            <div key={`${movie.id}-${idx}`} className="group relative flex flex-col cursor-pointer" onClick={() => openPlayer(movie)}>
              ...
              >
                <ArrowDownTrayIcon className="w-3 h-3 mr-1" /> empurrar download
              </button>
            </div>
          ))}
        </div>
        {loading && results.length > 0 && <div className="text-slate-400 text-center py-6 text-sm">Carregando mais...</div>}
        {!hasMore && results.length > 0 && !isSearchMode && <div className="text-slate-400 text-center py-6 text-sm">Acabou</div>}
      </div>
