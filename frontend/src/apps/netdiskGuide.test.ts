...
test('getNetdiskGuideData returns baidu-specific setup values', () => {
  ...
  assert.equal(result.title, 'Baidu SkyDisk')
  assert.equal(result.storageTypeLabel, 'Baidu SkyDisk')
...
test('getNetdiskGuideData returns quark-specific setup values', () => {
  ...
  assert.equal(result.title, 'Disco de rede Quark')
  assert.equal(result.storageTypeLabel, 'Disco de rede Quark')
