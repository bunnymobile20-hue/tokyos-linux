...
function formatWidgetNaturalDue(parsed: ReturnType<typeof parseNaturalTaskInput>): string | null {
  ...
  const dateLabel = dueDate.toLocaleDateString('pt-BR', { month: 'numeric', day: 'numeric' })
  if (parsed.isAllDay) {
    return `${dateLabel} o dia todo`
  }

  const timeLabel = dueDate.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
...
export default function DesktopWidgets({ onOpenDownloads, onOpenDida, authReady = false }: DesktopWidgetsProps) {
    ...
    const fetchDida = async () => {
      try {
        ...
        if (!status.success || !status.connected) {
          ...
          setDidaError(status.error || 'O tick não está autorizado')
          setDidaProjects([])
          setDidaTasks([])
          return
        }
        ...
        setDidaAuthorized(false)
        setDidaError('Falha na sincronização do tick')
        setDidaProjects([])
        setDidaTasks([])
      }
    }
  ...
  const handleCreateDidaTask = async () => {
    ...
    if (!parsed.title.trim()) {
      setDidaError('Por favor insira o conteúdo da tarefa')
      return
    }
    ...
    try {
      ...
      setDidaError('Falha ao criar tarefa')
    } finally {
  ...
  const handleToggleDidaTask = async (taskId: string) => {
    ...
    try {
      ...
      setDidaError('Falha na tarefa de atualização')
    } finally {
      ...
      <div className="grid grid-cols-2 auto-rows-[160px] gap-4 h-full pb-8">
        <div className="row-span-2 bg-white/40 backdrop-blur-xl border border-white/40 shadow-lg rounded-2xl px-4 pt-3 pb-3 text-slate-800 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between gap-3 mb-3">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-slate-500">{didaAuthorized ? `Pendentes: ${didaModel.pendingCount}` : 'Não autorizado'}</p>
            </div>
            <button
              type="button"
              onClick={onOpenDida}
          ...
          {didaAuthorized ? (
            <>
              <div className="rounded-xl bg-white/30 px-2.5 py-2 border border-white/25 mb-3">
                ...
                <input
                  ...
                  placeholder="Nova tarefa, ex: Reunião às 14:00"
                  ...
                  <div className="mt-1.5 flex items-center justify-between gap-2 text-[10px] min-h-4">
                    ...
                    {naturalDuePreview ? (
                      <span className="text-[#4a76f6] font-medium flex-shrink-0">
                        {naturalDuePreview}{didaDraftParsed.reminder ? ' lembrar' : ''}
                      </span>
                    ) : null}
                  </div>
                ) : null}
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-400 px-1 mb-1.5">
                ...
                <span>Concluído {didaModel.completedTodayCount}</span>
              </div>
              ...
              {didaModel.tasks.length > 0 ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center rounded-xl bg-white/20 text-slate-400 px-6 border border-white/10 border-dashed">
                  ...
                  <p className="text-[13px] font-medium text-slate-600 mb-0.5">Caixa de entrada limpa</p>
                  <p className="text-[11px]">Adicione uma nova tarefa</p>
                </div>
              )}
            </>
          ) : (
            ...
            >
              ...
              <p className="text-[13px] font-medium text-slate-700 mb-1">TickTick Desconectado</p>
              <p className="text-[11px] text-slate-500 leading-5">Após conectar, você poderá gerenciar tarefas diretamente pela área de trabalho.</p>
              <span className="mt-3 text-[11px] font-medium text-[#4a76f6]">Clique para autorizar</span>
            </button>
          )}
        ...
        <div className="h-full bg-white/40 backdrop-blur-xl border border-white/40 shadow-lg rounded-2xl p-5 text-slate-800 flex flex-col justify-center relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full justify-center">
            <h2 className="text-4xl font-light tracking-tight mb-1">{time.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</h2>
            <p className="text-sm font-medium text-slate-600 mb-3">{time.toLocaleDateString('pt-BR', { month: 'long', day: 'numeric', weekday: 'long' })}</p>
            <div className="flex items-center text-xs text-slate-500 bg-white/30 rounded-lg px-2 py-1.5 border border-white/20 w-fit max-w-[240px] truncate">
              <span className="font-semibold mr-1 flex-shrink-0">Tempo Livre:</span>
              <span className="truncate">
                {weather ? `${weather.label} ${Math.round(weather.temperature)}°C · umidade ${Math.round(weather.humidity)}%` : weatherError ? 'Clima indisponível' : 'Buscando clima...'}
              </span>
            </div>
            {weather ? <p className="mt-1.5 text-[10px] text-slate-400">vento {Math.round(weather.windSpeed)} km/h · atualizado às {weather.updatedAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p> : null}
          </div>
        </div>
              ...
              <div className="space-y-2">
                <div>
                  <div className="flex justify-between text-[10px] mb-0.5 font-medium leading-tight">
                    <span className="min-w-0 pr-2">Uso da CPU</span>
                ...
                <div>
                  <div className="flex justify-between text-[10px] mb-0.5 font-medium leading-tight">
                    <span className="min-w-0 pr-2">Memória</span>
                ...
                <div>
                  <div className="flex justify-between text-[10px] mb-0.5 font-medium leading-tight">
                    <span className="min-w-0 pr-2">Armazenamento do sistema</span>
        ...
        >
          <div className="flex items-center justify-between gap-3 mb-3">
            <div className="flex items-center space-x-2 min-w-0">
              ...
              <h3 className="text-sm font-bold">Fila de Downloads</h3>
            </div>
            <span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${downloadAvailable ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
              {downloadAvailable ? 'Motor Online' : 'Motor Offline'}
            </span>
          </div>
          <div className="flex flex-wrap gap-1 mb-2 text-[10px] font-medium text-slate-600">
            <span className="px-2 py-0.5 rounded-full bg-cyan-50 text-cyan-700">Baixando {downloadCounts.active}</span>
            <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-700">Fila {downloadCounts.waiting}</span>
            <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">Pausado {downloadCounts.paused}</span>
            <span className="px-2 py-0.5 rounded-full bg-red-50 text-red-700">Erro {downloadCounts.error}</span>
          </div>
          <div className="flex-1 flex flex-col justify-center space-y-2 overflow-hidden">
            {downloads.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center text-xs text-slate-500 bg-white/20 rounded-lg border border-white/10 border-dashed text-center px-4">
                {downloadAvailable ? 'Nenhum download ativo' : 'Mecanismo de download indisponível'}
              </div>
            )}
          </div>
        </button>
        ...
        <div className="h-full bg-white/40 backdrop-blur-xl border border-white/40 shadow-lg rounded-2xl p-4 text-slate-800 overflow-hidden relative flex flex-col group">
          <div className="flex items-center justify-between mb-2 relative z-10">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-bold">{musicState?.appId === 'localmusic' ? 'Música Local' : musicState?.appId === 'music' ? 'NetEase Music' : 'Tocando Agora'}</h3>
            </div>
            {musicState ? (
            ...
            {musicState ? (
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <p className="text-base font-black truncate text-slate-800 drop-shadow-sm mb-0.5" title={musicState.title}>{musicState.title || 'Título desconhecido'}</p>
                  <p className="text-xs text-slate-600 font-medium truncate drop-shadow-sm mb-1" title={musicState.artist}>{musicState.artist}</p>
                  <p className="text-[10px] text-blue-600 font-medium truncate drop-shadow-sm h-4">{musicState.lyric || (musicState.playing ? '♪ Curtindo a música...' : '')}</p>
              ...
              <div className="h-full flex items-center justify-center space-x-3 text-slate-500 p-2 bg-white/20 rounded-xl border border-white/10 border-dashed">
                ...
                <span className="text-xs font-medium">Abra o aplicativo de música</span>
              </div>
            )}
          </div>
