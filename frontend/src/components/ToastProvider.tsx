      ...
      <div className="flex-1 min-w-0 pr-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            {isCritical ? <span className="px-1.5 py-0.5 rounded bg-red-100 text-red-700 text-[10px] font-bold">alto risco</span> : null}
            <h4 className={`text-sm font-semibold truncate ${isCritical ? 'text-red-900' : 'text-slate-800'}`}>{notification.title}</h4>
          </div>
          <span className={`text-[10px] ${isCritical ? 'text-red-400' : 'text-slate-400'}`}>{behavior.stickyToasts || isCritical ? 'permanente' : 'apenas'}</span>
        </div>
