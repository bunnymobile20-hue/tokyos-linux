...
async function flushPendingBatch(batchKey: string) {
  ...
  const batchedMessage = entry.count > 1
    ? entry.messageBuilder?.(entry.count, entry.latestMessage) || `${entry.latestMessage}（No período recente, um total de ${entry.count} Segunda categoria）`
    : entry.latestMessage
...
export const useNotificationStore = create<NotificationStore>((set) => ({
  ...
  init: async () => {
    ...
    notificationUnsubscribe = subscribeSystemNotifications({
      ...
      onError: () => {
        // SSE Pode ser bloqueado devido à política de autenticação，Abaixo está o guia de votação
      },
    })
    ...
    notificationPollTimer = window.setInterval(() => {
      void fetchSystemNotifications({ includeRead: true, limit: 100 })
        ...
        .catch(() => {
          // A falha na votação não bloqueia o processo principal
        })
    }, 6000)
  },
